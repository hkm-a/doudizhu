...
from api.player_event_summary import (
    format_summary_html,
    format_summary_text,
    summarize_player_event_log,
)


def main() -> int:
    parser = argparse.ArgumentParser(description='Summarize player behavior JSONL logs.')
    parser.add_argument('path', nargs='?', default=os.getenv('PLAYER_EVENT_LOG_PATH'))
    ...
    if not args.path:
        parser.error('path is required, or set PLAYER_EVENT_LOG_PATH')

    summary = summarize_player_event_log(args.path)
    if args.html:
